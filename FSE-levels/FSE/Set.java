...
class Set extends JPanel implements KeyListener, ActionListener, MouseListener {
	...
	java.awt.Image pic;
	private int mx, my;
	int INTRO = 0, HOWTOUSE = 1, CREATE = 2;
	int screen;
	...
	public void paint(Graphics g) {
		if (screen == INTRO) {
			...
			g.drawString("La Boutique", 630, 150);
			g.drawImage(new ImageIcon("effiel tower resize.png").getImage(), 6, 230, null);
			g.drawImage(new ImageIcon("arc resize.png").getImage(), 1350, 300, null);
			g.drawImage(new ImageIcon("beret_250.png").getImage(), 40, 150, null);
			g.drawImage(new ImageIcon("Logo_690.png").getImage(), 610, 230, null);
		}
		if (screen == CREATE) {
			Graphics2D g2d = (Graphics2D) g;
